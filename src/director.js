...
// 망설이는 사람은 slack 이 0 이라 바닥을 받는다(그것이 망설임에 맞서는 방법이다).
// 그 자리에서 실여유가 정확히 이 값이다. 가장 많이 헛디디는 사람이 가장 적은
// 여유를 받으므로, 여기가 인색하면 조이는 것이 아니라 못 끝내게 만드는 것이 된다.
//
// 2 로 두었다가 4 로 올렸다. 시드 200개 × 의도 3종을 1~6 으로 재 봤다.
//
//   margin   창바닥여유   지형폭   착시강제   NO_BUDGET_WINDOW   평균 시도
//        2       2칸       8.0칸     100%            4%            3.2
//        4       4칸       8.0칸     100%           16%            3.5
//        6       6칸      10.0칸     100%           36%            4.4
//
// 착시를 강제하는 성질은 값과 무관하게 유지된다. verify 가 착시 층에
// honestCost > inkMax 를 요구하므로 통과한 층은 정의상 강제되고, margin 이
// 하는 일은 그 배급을 찾기 어렵게 만드는 것뿐이다. 못 찾으면 지형을 다시
// 뽑는다 — **압박이 약해지는 것이 아니라 재설계가 느는 것**이다.
//
// 6 은 안 골랐다. 재설계 36% 도 비싸지만 지형폭 중앙값이 8 에서 10 으로
// 밀리는 것이 문제다. 폭이 좁은 지형이 통째로 떨어져 나가 짧은 이음매 층이
// 사라지고, 그러면 층의 성격 자체가 한쪽으로 쏠린다. 4 까지는 8.0 그대로다.
export const WASTE_MARGIN = 4;   // 칸
